        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px]">
            ...
            <div className="relative w-full max-w-[500px] h-[610px] bg-white dark:bg-zinc-950 rounded-none border-b-[10px] border-[#46A358] shadow-2xl flex flex-col justify-between z-10 text-[#3D3D3D] dark:text-zinc-300 font-sans p-10 pt-12 animate-in fade-in zoom-in-95 duration-200">                
                <div>
                    ...
                    <div className="flex items-center justify-center gap-3 text-lg font-medium mb-6">
                        <span 
                        ...
                        <span className="text-[#46A358] font-bold cursor-pointer">{t("text218")}</span>
                    </div>
                    <p className="text-zinc-500 dark:text-zinc-400 text-xs font-normal mb-3">
                        {/* Здесь можно использовать "Enter your email and password to register." напрямую или добавить новый ключ, пока оставил плейсхолдер структуры */}
                        Enter your email and password to register.
                    </p>
                    <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-3 w-full">
                        <input 
                            type="text" 
                            placeholder={t("text226")}
                        ...
                        <input 
                            type="email" 
                            placeholder={t("text227")}
                            className="w-full h-10 px-3 text-sm bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md focus:outline-none focus:border-[#46A358] text-zinc-700 dark:text-zinc-200" />
                        <div className="relative w-full">
                            <input 
                                type={showPassword ? "text" : "password"} 
                                placeholder={t("text228")}
                        ...
                        <Link 
                            to="/account"
                            onClick={onClose}
                            className="w-full bg-[#46A358] hover:bg-[#3b8a4a] text-white py-2.5 rounded-md font-bold text-sm transition-colors shadow-sm mt-1 flex items-center justify-center"
                        >
                            {t("text218")}
                        </Link>
                    </form>
                    <div className="relative flex py-3 items-center my-0.5">
                        <div className="flex-grow border-t border-zinc-100 dark:border-zinc-900"></div>
                        <span className="flex-shrink mx-4 text-zinc-400 dark:text-zinc-500 text-xs font-normal">Or register with</span>
                        <div className="flex-grow border-t border-zinc-100 dark:border-zinc-900"></div>
                    </div>
                </div>
